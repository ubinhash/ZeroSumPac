...
const EquipKeys = ({ contracts,currplayerid, network ='shape-sepolia',setPopupMsg}) => {
    ...
    const {
        ...
        functionName: 'equipKeys', // Function name
    ...
    useEffect(() => {
        const fetchNFTs = async () => {
            setLoading(true);
            try {
                const response = await fetch(`${webconfig.apiBaseUrl}/getKeyByOwner?owner=${address}&network=${network}&withMetadata=false`);
    ...
    return (
        <div className={styles.container}>
            <h1 className={styles.title}>Equip Keys To Enter Special Maze</h1>
                ...
                <>
                    {nfts.length > 0 ? (
                        ...
                        <div className={styles.infoText}>
                            You hold <span className={styles.hightlight}>{nfts.length}</span> key
                        </div>
                        <div className={styles.infoText}>Please select a token: </div>
                        </>
                    ) : (
                        <p className={styles.infoText}>You don't hold any Shapecraft Key NFT yet.</p>
                    )}
